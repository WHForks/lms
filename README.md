...
Local environment uses Nix and only supports Nix as an official method. Consult [Nix installation instructions](https://nixos.org/download.html) if you don't have Nix installed yet.
...
2. Run `yarn install`
3. Run `yarn run local:1` to build the frontend code.
4. Run `yarn run build:css` to only build the CSS. `local:1` task also runs this command.
